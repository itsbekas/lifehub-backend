...
def obj_oldest_user1(user_id1, date_oldest):
    return TimeUserBaseTestModel(user_id=user_id1, timestamp=date_oldest, text="obj_oldest1")

@pytest.fixture(scope="function")
def obj_latest_user1(user_id1, date_latest):
    return TimeUserBaseTestModel(user_id=user_id1, timestamp=date_latest, text="obj_latest1")

@pytest.fixture(scope="function")
def obj_oldest_user2(user_id2, date_oldest):
    return TimeUserBaseTestModel(user_id=user_id2, timestamp=date_oldest, text="obj_oldest2")

@pytest.fixture(scope="function")
def obj_latest_user2(user_id2, date_latest):
    return TimeUserBaseTestModel(user_id=user_id2, timestamp=date_latest, text="obj_latest2")
...
class TestGetLatest:
    def test_empty(self, db_client):
        ...
        """
        assert db_client.get_latest() == None
    ...
    def test_single_incorrect_user(self, db_client, obj_latest_user2):
        ...
        Expected: None is returned
        """
        user2_db_client = TimeUserBaseDBClient(TimeUserBaseTestModel, obj_latest_user2.user_id)
    ...
    def test_multiple_incorrect_user(self, db_client, obj_oldest_user1, obj_latest_user2):
        """
